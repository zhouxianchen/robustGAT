"""
The basic GAT with weight model of original article.
"""
import time
import numpy as np
    ...
    def fit(self, features, adj, labels, idx_train, idx_val):
        ...
        for epoch in range(args.epochs):
            if args.only_gcn:
                ...
                print("train_gcn time: ", time.time()-t0)
            else:
                t0 = time.time()
                for i in range(int(args.outer_steps)):
                    self.train_adj(epoch, features, adj, labels,
                            idx_train, idx_val)
                print("train_adj time: ", time.time() - t0)

                t1 = time.time()
                for i in range(int(args.inner_steps)):
                    self.train_gcn(epoch, features, estimator.estimated_adj,
                            labels, idx_train, idx_val)
                print("train_gcn time: ", time.time()-t1)
